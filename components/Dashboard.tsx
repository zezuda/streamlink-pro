...
const StatusIndicator: React.FC<{ platform: string, stats: StreamStats, icon: React.ReactNode }> = ({ platform, stats, icon }) => {
      ...
      <main className="flex-1 flex flex-col md:flex-row h-full overflow-hidden">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-lg font-black text-white flex items-center gap-2 tracking-tight">
                  STREAM FEED <span className={`flex h-2 w-2 rounded-full ${state.messages.length > 0 ? 'bg-emerald-500 animate-pulse' : 'bg-red-500'}`}></span>
                </h1>
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{unreadCount} New interactions</p>
              </div>
            </div>
        ...
        <section className="w-full md:w-80 lg:w-96 bg-slate-950 flex flex-col p-6 space-y-6 overflow-y-auto custom-scrollbar">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <StatusIndicator 
                ...
                icon={<svg className="w-3 h-3 text-purple-500 fill-current" viewBox="0 0 24 24"><path d="M11.571 4.714h1.715v5.143H11.57V4.714zm4.715 0H18v5.143h-1.714V4.714zM6 0L1.714 4.286v15.428h5.143V24l4.286-4.286h3.428L22.286 12V0H6zm14.571 11.143l-3.428 3.428h-3.429l-3 3v-3H6.857V1.714h13.714v9.429z"/></svg>} 
              />
              <StatusIndicator 
                platform="YouTube" 
          ...
          <div className="mt-auto pt-4 border-t border-slate-900">
            <QuotaTracker used={state.quotaUsage} onEdit={onOpenSettings} />
          </div>
        </section>
      </main>
    </div>
